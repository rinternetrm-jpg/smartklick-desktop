...
class WhisperClient:
    ...
    async def _parse_sse_response(
        ...
        async for line in response.content:
            ...
            # Debug: Log all lines from server
            if line:
            ...
            logger.info(f"SSE data: {data_str}")
            ...
            try:
                ...
                # Handle phase events from server
                phase = data.get('phase', '')

                if phase == 'done':
                    # Final response with rawText and cleanedText
                    full_text = data.get('rawText', data.get('text', ''))
                    cleaned = data.get('cleanedText', '')
                    if data.get('jarvisResult', False) and cleaned:
                        jarvis_response = cleaned
                        is_jarvis_command = True
                    elif cleaned:
                        full_text = cleaned
                    logger.info(f"Done: rawText='{full_text}', cleanedText='{cleaned}', jarvis={is_jarvis_command}")

                elif event_type == 'text' or 'text' in data:
                    # Text chunk received
                    ...
                    # Final response (alternative format)
                    if 'text' in data:
                        full_text = data['text']
